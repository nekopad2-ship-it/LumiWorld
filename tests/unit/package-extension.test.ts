...
test("packaging copies a clean installable tree without .git", () => {
  ...
  writeFileSync(
    join(sourceDir, "node_modules", "leftpad", "index.js"),
    "module.exports = 1;",
  );
  ...
  assert.equal(
    readFileSync(
      join(result.repoDir, "src", "backend", "storage", "spindle-storage.ts"),
      "utf8",
    ),
    "export const marker = true;\n",
  );
  assert.throws(() =>
    readFileSync(join(result.repoDir, ".git", "objects", "pack.idx"), "utf8"),
  );
  assert.throws(() =>
    readFileSync(
      join(result.repoDir, "node_modules", "leftpad", "index.js"),
      "utf8",
    ),
  );
});
